...
import { ACTIVE_EVENT_TYPES, RESERVED_EVENT_TYPES, EventType, isActiveEventType, isReservedEventType, isEventType, sanitizeMetadata, SAFE_METADATA_KEYS, type ActiveEventType, type EventUserContext, type ReservedEventType, type SafeMetadataKey } from './types'
...
export async function trackActiveEvent(opts: {
  ...
  if (!isActiveEventType(opts.eventType)) {
    if (IS_DEV) {
      // eslint-disable-next-line no-console
      console.warn(`[event-tracker] "${opts.eventType}" is not an active event type. Use trackEvent with a reserved type for Phase 4.3.`)
    }
    return
  }
  const safeMetadata = sanitizeMetadata(opts.metadata ?? {})
...
if (typeof window !== 'undefined' && IS_DEV && (window as any).__EVENT_TRACK_DEBUG__) {
  // eslint-disable-next-line no-extend-native
  const orig = trackEvent
 ;(trackEvent as any) = async (opts: any) => {
